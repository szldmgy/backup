...
\begin{Prob}[等式约束的距离几何问题, 记为 eDGP]
...
\begin{Prob}[不等式约束的距离几何问题, 记为 iDGP]
对于图$G=(V,E)$, 其中$V$是顶点集合, $E$是边的集合, 
...
\begin{figure}[htbp]
  \centering
  \subfigure{ \includegraphics[width=0.45\textwidth]{GraphRealization.eps} }
  \subfigure{ \includegraphics[width=0.45\textwidth]{GraphRealization2.eps} }
  \caption{二维平面上的画图问题}
...
\begin{figure}[htbp]
  \centering
  \subfigure{ \includegraphics[width=0.45\textwidth]{1PTQ.eps} }
...
\begin{figure}[htbp]
  \centering
  \subfigure{ \includegraphics[width=0.45\textwidth]{adhoc.eps} }
  \subfigure{ \includegraphics[width=0.45\textwidth]{WirelessSensorNetwork.png} }
  \caption{传感器网络示意图}
  \label{fig:sensor}
\end{figure}
...
\vskip3mm
\begin{lstlisting}  
function [X,Fval] = PostProcess(D,X0,options)
%%  PostProcess applys Barzilai-Borwein methold to ajust the points, such 
%%  that they fit the distances better. It is the enhanced version on Page 
%%  52 of the book by Ya-Xiang Yuan (2007).
%%
%%  The objective function is the stress function:
%%    f(x1,x2,...,xn) = \sum( (||xi-xj|| -dij )^2 )
%%  X0 = [x1 x2 ... xn]'
% parameter settings
n = size(X0,1);
if nargin<3
    ...
    options.alg = 'LBB';
    options.tol = 10^(-2);
    options.maxitr = min(n,200);
else
    if ~isfield(options,'display'),  options.display = 0; end
    if ~isfield(options,'Mstep'),    options.Mstep = 4;   end
    if ~isfield(options,'alg'),      options.alg = 'LBB'; end
    if ~isfield(options,'tol'),      options.tol = 10^(-2); end
    if ~isfield(options,'maxitr'),   options.maxitr = min(n,200); end
end
b = 10^(-4);  
fun = @stress; gfun = @gstress;
% Recod all the edges
[I,J] = find(triu(D));  
% Recod the neighbourhood of each point
neib = zeros(n,60); nneib = zeros(n,1);
for ii = 1:n
    nz = find(D(ii,:));
    nneib(ii) = length(nz);
    neib(ii,1:nneib(ii)) = nz;
end
%% BB iteration 
f0 = fun(X0); g0 = gfun(X0); alpha = 10^(-2);  k = 1; index = 1;
jj = 0;
while jj<10
    X1 = X0 - alpha * g0;
    f1 = fun(X1);
    if f1<f0-alpha*b*norm(g0,'fro')^2,  break; end
    alpha = alpha/2;
    jj = jj + 1;
end
g1 = gfun(X1); fmin = f1;
while k<options.maxitr
    if norm(g1,'fro')<options.tol || abs(f0-f1)<options.tol,  break;  end
    s = X1 - X0; y = g1 -g0;
    switch options.alg
        case 'SBB' % short BB stepsize
            alpha = sum(sum(s.*y)) / norm(y,'fro')^2;
        case 'LBB' % long BB stepsize
            alpha = norm(s,'fro')^2 / sum(sum(s.*y));
        case 'MBB' % cycled stepsize
            if mod(k,2)==1
                alpha = sum(sum(s.*y)) / norm(y,'fro')^2;
            else alpha = norm(s,'fro')^2 / sum(sum(s.*y));
            end
    end
    % Backtraking line search with BB step as intial stepsize 
    if k-index==options.Mstep || alpha<0
        if alpha<0, alpha = 10^(-2); end
        jj = 0;
        while jj<10
            X0 = X1 - alpha * g1;
            f0 = fun(X0);
            if f0<fmin-alpha*b*norm(g1,'fro')^2,  break; end
            alpha = alpha/2;
            jj = jj + 1;
        end
    end
    X0 = X1;  X1 = X1 - alpha * g1;
    f0 = f1;  f1 = fun(X1);
    g0 = g1;  g1 = gfun(X1);
    k = k + 1;
    if f1<fmin, index = k; fmin = f1; end
end
X = X1;  Fval = f1;
if options.display==1, fprintf('%3d ',k); end
if options.display==2, fprintf('%3d %4.1e',k,Fval);end

%% stress function evaluation 
    function f = stress(X)
        f = sum((sqrt(sum((X(I,:)-X(J,:)).^2,2)) - D(I+(J-1)*n)).^2) / 2;
    end
%% stress gradient evaluation 
    function g = gstress(X)
        g= zeros(size(X));
        for i = 1:n
            j = neib(i,1:nneib(i));
            temp = ones(nneib(i),1)*X(i,:)-X(j,:);
            g(i,:) =  (1-D(j,i)./sqrt(sum((temp).^2,2)))' * temp ;
        end
    end
\end{lstlisting}
